...
router.post("/register", validInfo, async (req, res) => {
    try {
       //1. destructure the req.body for member registration values (businessname, email, password etc)
        ...
        const {businessname, businessno, email, password,
             phone, state, city, address, postcode} = req.body;

       //2. check if the member exists (If so, then throw an error)
        const member = await pool.query("SELECT * FROM member WHERE member_email = $1", [
        email
        ]);
       ...
       //4. insert the new member into the db
        const newMember = await pool.query(
            ...
            [businessname, businessno, email, bcryptPassword, phone, state, city, address, postcode]
        );
...
router.post("/login", validInfo, async (req, res) => {
    try {
        ...
        const validPassword = await bcrypt.compare(password, member.rows[0].member_password);
